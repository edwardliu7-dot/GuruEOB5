...
import { FadeIn, StaggerContainer, StaggerItem, ScaleIn, NumberTicker } from "@/components/motion";
  ...
  return (
    <Layout>
      <div className="space-y-8">
        ...
        {/* Stats Row — staggered cards */}
        <StaggerContainer className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StaggerItem>
            <Card className="border-none shadow-sm ring-1 ring-black/5 card-hover">
                  ...
                  <div>
                    ...
                    {isLoading ? <Skeleton className="h-8 w-16 mt-1" /> : (
                      <h2 className="text-3xl font-bold">
                        <NumberTicker value={summary?.totalSiswa ?? 0} />
                      </h2>
                    )}
                  </div>
                </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {isLoading ? <Skeleton className="h-8 w-16 mt-1" /> : (
                      <h2 className="text-3xl font-bold">
                        <NumberTicker value={summary?.totalGuru ?? 0} />
                      </h2>
                    )}
                  </div>
                </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {isLoading ? <Skeleton className="h-8 w-16 mt-1" /> : (
                      <h2 className="text-3xl font-bold">
                        <NumberTicker value={summary?.totalDokumen ?? 0} />
                      </h2>
                    )}
                  </div>
                </div>
          ...
          <StaggerItem>
            <Card className={`border-none shadow-sm ring-1 ring-black/5 card-hover relative overflow-hidden ${summary?.jurnalHariIniTerisi ? 'bg-emerald-500' : 'bg-rose-500'}`}>
            ...
            <Card className="border-none shadow-sm ring-1 ring-black/5 h-full">
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <ScaleIn className="h-[300px] w-full">
            ...
            <Card className="border-none shadow-sm ring-1 ring-black/5 h-full">
              ...
              <CardContent className="flex flex-col items-center justify-center pt-8 pb-12">
                {isLoading ? (
                  ...
                  <ScaleIn delay={0.15} className="w-full flex flex-col items-center">
                    <div className="relative w-40 h-40 flex items-center justify-center mb-8">
                      ...
                      <div className="absolute flex flex-col items-center justify-center">
                        <span className="text-4xl font-bold font-serif">
                          <NumberTicker value={summary?.kelengkapanAdministrasiPersen ?? 0} />%
                        </span>
                      </div>
                    </div>
